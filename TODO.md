...
This file is a braindump of ideas to improve Quatrosec2 (or forward-looking to "Quatrosec3")

------------------------------------------------------

### Better Dependency Handling
I can rely on `pip` + `requirements.txt` for python libraries, but most of quatrosec's dependencies are installed programs.

When a dependency is not found, Quatrosec should walk the user through installing all required dependencies, and maybe the optional dependencies as well.
...
Since we have a Python script for every dependency (under `quatrosec/tools/` or `quatrosec/util/`), we use Python's multiple-inheritance to achieve this.
...
3. Dependency-checker to run at Quatrosec startup.
...
* airmon-ng output differs, quatrosec sees "phy0" instead of the interface name.
...
* Some people want quatrosec to kill network manager, others don't.
...
Quatrosec is a 'Spray and Pray', 'Big Red Button' script. Quatrosec should not provide obscure options that only advanced users can understand. Advanced users can simply use Quatrosec's dependencies directly.

--------------------------------

Every option in Quatrosec's should either:

1. Significantly affect how Quatrosec behaves (e.g. `pillage`, `5ghz`, '--no-wps', '--nodeauths')
...
Some dependencies of Quatrosec (aircrack suite, tshark, etc) could be replaced with native Python implementations.
...
Some of Quatrosec's dependencies work on other OSes (airodump) but some don't (airmon).

If it's possible to run these programs on Windows or OSX, Quatrosec should support that.

------------------------------------------------------

### WPS Attacks

Quatrosec's Pixie-Dust attack status output differs between Reaver & Bully. And the command line switches are... not even used by bully?
...
And as for PIN cracking.. um.. Not even sure this should be an option in Quatrosec TBH.
PIN cracking takes days and most APs auto-lock after 3 attempts.
Multi-day (possibly multi-month) attacks aren't a good fit for Quatrosec.
