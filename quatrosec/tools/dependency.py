...
class Dependency(object):
    ...
    def run_dependency_check(cls):
        ...
        if missing_required:
            Color.pl('{!} {O}لانیکەم 1 پڕۆگرامی پێویست کەمن. کواترۆسێک پێویستی پڕۆگرامە پێویستەکان هەیە بۆ بەڕێوەچوون{W}')
            import sys
            sys.exit(-1)
    ...
    def fails_dependency_check(cls):
        ...
        if cls.dependency_required:
            ...
            Color.pl('. {W}دامەزراندن @ {C}%s{W}' % cls.dependency_url)
            return True

        else:
            Color.p('{!} {O}ئاگاداری: پڕۆگرامی ڕاسپاردە {R}%s{O} نەدۆزرایەوە' % cls.dependency_name)
            Color.pl('. {W}دامەزراندن @ {C}%s{W}' % cls.dependency_url)
            return False
