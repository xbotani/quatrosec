    ...
    def terminate_conflicting_processes():
        ...
        for pid, pname in pid_pnames:
            if pname == 'NetworkManager' and Process.exists('service'):
                Color.pl('{!} {O}وەستاندنی network-manager ({R}service network-manager stop{O})')
                ...
                Color.pl('{!} {O}وەستاندنی avahi-daemon ({R}service avahi-daemon stop{O})')
                # Can't just pkill avahi-daemon; it's a service
                Process(['service', 'avahi-daemon', 'stop']).wait()
            else:
                Color.pl('{!} {R}کۆتایی هێنان بە {O}پڕۆسەی ناکۆک {R}%s{O} (PID {R}%s{O})' % (pname, pid))
                try:
