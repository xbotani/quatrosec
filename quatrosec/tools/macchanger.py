...
class Macchanger(Dependency):
    ...
    def down_macch_up(cls, iface, options):
        ...
        macch.wait()
        if macch.poll() != 0:
            Color.pl('\n{!} {R}macchanger{O}: هەڵە لە بەڕێوەبردنی {R}%s{O}' % ' '.join(command))
            Color.pl('{!} {R}دەرئەنجام: {O}%s, %s{W}' % (macch.stdout(), macch.stderr()))
            return False
    ...
    def reset(cls):
        ...
        Color.pl('\r{+} {C}macchanger{W}: ڕێکخستنەوەی ناونیشانی ماک لە %s...' % iface)
        # -p to reset to permanent MAC address
        if cls.down_macch_up(iface, ['-p']):
            new_mac = Ifconfig.get_mac(iface)

            Color.clear_entire_line()
            Color.pl('\r{+} {C}macchanger{W}: ڕێکخستنەوەی ناونیشانی ماک بۆ {C}%s{W} لە {C}%s{W}' % (new_mac, iface))
    ...
    def random(cls):
        ...
        if not Process.exists('macchanger'):
            Color.pl('{!} {R}macchanger: {O}دامەزراو نییە')
            return

        iface = cls.get_interface()
        Color.pl('\n{+} {C}macchanger{W}: گۆڕینی ناونیشانی ماک لە {C}%s{W}' % iface)
        ...
        if cls.down_macch_up(iface, ['-e']):
            ...
            Color.clear_entire_line()
            Color.pl('\r{+} {C}macchanger{W}: ناونیشانی ماک گۆڕدرا بۆ {C}%s{W} لە {C}%s{W}' % (new_mac, iface))
