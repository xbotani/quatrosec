...
class Scanner(object):
    ...
    def found_target(self):
        ...
        if self.target:
            Color.pl('\n{+} {C}ئامانج دۆزرایەوە{G} %s {W}({G}%s{W})'
    ...
    def print_targets(self):
        ...
        if Configuration.show_bssids:
            Color.p('              BSSID')
        Color.pl('   کەناڵ  شێوەزار  هێز  WPS?  کڵایەنت')
    ...
    def select_targets(self):
        ...
        input_str  = '{+} هەڵبژاردنی ئامانج(ەکان)'
        input_str += ' ({G}1-%d{W})' % len(self.targets)
        input_str += ' بە کۆما، خەت'
        input_str += ' یان {G}هەموو{W}: '

        chosen_targets = []

        for choice in raw_input(Color.s(input_str)).split(','):
            choice = choice.strip()
            if choice.lower() in ['all', 'هەموو']:
