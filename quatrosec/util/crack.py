...
class CrackHelper:
    ...
    def run(cls):
        ...
        if not Configuration.wordlist:
            ...
            if not os.path.exists(Configuration.wordlist):
                Color.pl('{!} {R}لیستی وشەکان {O}%s{R} نەدۆزرایەوە. دەرچوون.' % Configuration.wordlist)
                return
            Color.pl('')
        ...
        if len(handshakes) == 0:
            Color.pl('{!} {O}هیچ دەست لێدان نەدۆزرایەوە{W}')
            return
        ...
        if len(missing_tools) > 0:
            Color.pl('\n{!} {O}ئامرازە بەردەست نەبووەکان (دامەزراندن بۆ چالاککردن):{W}')
            for tool, deps in missing_tools:
                dep_list = ', '.join([dep.dependency_name for dep in deps])
                Color.pl('     {R}* {R}%s {W}({O}%s{W})' % (tool, dep_list))

        if all_pmkid:
            Color.pl('{!} {O}تێبینی: PMKID هاشەکان تەنها بە {C}hashcat{W} دەکرێت شیکردنەوە')
            tool_name = 'hashcat'
        else:
            ...
            if tool_name not in available_tools:
                Color.pl('{!} {R}"%s"{O} ئامراز نەدۆزرایەوە، گەڕان بۆ {C}aircrack{W}' % tool_name)
                tool_name = 'aircrack'
            ...
            for hs in hs_to_crack:
                if tool_name != 'hashcat' and hs['type'] == 'PMKID':
                    if 'hashcat' in missing_tools:
                        Color.pl('{!} {O}Hashcat کەمن، بۆیە ناتوانین PMKID هاش شیکردنەوە{W}')
    ...
    def get_handshakes(cls):
        ...
        if not os.path.exists(hs_dir) or not os.path.isdir(hs_dir):
            Color.pl('\n{!} {O}فۆڵدەر نەدۆزرایەوە: {R}%s{W}' % hs_dir)
            return []

        Color.pl('\n{+} لیستکردنی دەست لێدانە دەستگیرکراوەکان لە {C}%s{W}:\n' % os.path.abspath(hs_dir))
        ...
        if skipped_pmkid_files > 0:
            Color.pl('{!} {O}پەسەندکردنی %d {R}*.16800{O} فایلەکان چونکە {R}hashcat{O} کەمن.{W}\n' % skipped_pmkid_files)
        if skipped_cracked_files > 0:
            Color.pl('{!} {O}پەسەندکردنی %d فایلە شیکردنەوەکراوەکان.{W}\n' % skipped_cracked_files)
    ...
    def crack(cls, hs, tool):
        ...
        if crack_result is None:
            # Failed to crack
            Color.pl('{!} {R}شکست لە شیکردنەوە {O}%s{R} ({O}%s{R}): وشەی نهێنی لە فەرهەنگدا نییە' % (
                hs['essid'], hs['bssid']))
        else:
            # Cracked, replace existing entry (if any), or add to
            Color.pl('{+} {G}شیکردنەوە{W} {C}%s{W} ({C}%s{W}). کلیلی: "{G}%s{W}"' % (
